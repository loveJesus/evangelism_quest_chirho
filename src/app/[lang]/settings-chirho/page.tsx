...
export default function SettingsPageChirho({ params: { lang } }: SettingsPagePropsChirho) {
  const { fontSizeChirho, setFontSizeChirho, themeChirho, setThemeChirho, effectiveThemeChirho } = useCustomizationChirho();
  const [dictionary, setDictionary] = useState<DictionaryChirho['settingsPage'] | null>(null);

  useEffect(() => {
    const fetchDict = async () => {
      const d = await getDictionaryChirho(lang);
      setDictionary(d.settingsPage);
    };
    fetchDict();
  }, [lang]);
